...
def fmt_date(iso: str) -> str:
    """Convert ISO UTC date to Asia/Dhaka 12-hour format."""
    dt = datetime.fromisoformat(iso.replace("Z", "+00:00"))
    bst = dt.astimezone(DHAKA)
    return bst.strftime("%-d %B %Y, %I:%M %p BST")


def now_dhaka() -> str:
    """Current time in Asia/Dhaka 12-hour format."""
...
def build_api_section(d: dict) -> str:
    """API section keeps raw ISO date in JSON response - no conversion."""
    payload = {
        "success": True,
        "data": {
...
def build_update_table() -> str:
    """Table showing when this action last ran."""
    return (
        "| Field | Value |\n"
        "|-------|-------|\n"
        f"| Last Auto-Update | {now_dhaka()} |\n"
...
def build_svg(d: dict) -> str:
    ...
    today = datetime.now(DHAKA).strftime("%-d %B %Y")
    ...
    lines = [
        ...
        f'  <text x="350" y="50" text-anchor="middle" fill="#8b949e" font-family="\'Segoe UI\',Arial,sans-serif" font-size="11">Last updated: {today} (BST)</text>',
...
def main():
    ...
    update_table = build_update_table()
    svg          = build_svg(data)
    ...
    readme = replace_section(readme, "UPDATED", update_table)
